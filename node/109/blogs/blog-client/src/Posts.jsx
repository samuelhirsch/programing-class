...
export default function Posts({ setError }) {
  ...
  useEffect(() => {
    ...
    function addComment(comment) {
      const { _id, ...commentToPush } = comment;
      const newArrayForPosts = [...posts];
      const postToAddComment = newArrayForPosts.find((p) => p._id === comment.id);

      if (!postToAddComment.comments) {
        postToAddComment.comments = [];
      }
      postToAddComment.comments.push(commentToPush)
      setPosts(newArrayForPosts)
      //post.comments.push()
